...
parser = argparse.ArgumentParser(description='Masked Autoencoder Video Reconstruction Visualization')
parser.add_argument('--video_path', type=str, required=True, help='Path to input video file')
parser.add_argument('--ckpt_path', type=str, required=True, help='Path to model checkpoint file')
parser.add_argument('--cfg_file', type=str, required=True, help='Path to config YAML file')
parser.add_argument('--output_dir', type=str, default='/content/output', help='Path to output directory (default: /content/output)')
cmd_args = parser.parse_args()

# --- CONFIG ---
VIDEO_PATH = cmd_args.video_path
CKPT_PATH = cmd_args.ckpt_path
CFG_PATH = cmd_args.cfg_file
...
norm_mean = [0.45, 0.45, 0.45]  # From your config
...
if __name__ == "__main__":
    ...
    for mask_ratio in MASK_RATIOS:
        with torch.no_grad():
            ...
            # Determine which frames to use (no subsampling since TIME_STRIDE_LOSS=False)
